    ...
    public = base::list(
        ...
        plot.approximations = function(
            ...
            my.ggplot <- initializePlot(
              ...
              # subtitle = paste0("year: ",     temp.year,", location: ",temp.location)
                subtitle = paste0(string.annee, temp.year,string.coords,temp.location)
                );

          # my.ggplot <- my.ggplot + ggplot2::xlab( label = "date index (1 = New Year's Day)" );
            my.ggplot <- my.ggplot + ggplot2::xlab( label = "indice de date (1 = Jour de l'An)" );
        ...
        plot.harmonics = function(
            ...
            for ( temp.index in base::seq(1,base::length(temp.harmonics)) ) {
                ...
                temp.string <- " (variabilité capturée = ";
                Encoding(temp.string) <- "UTF-8";

                my.ggplot <- initializePlot(
                    title    = NULL,
                  # subtitle = base::paste0(self$variable," (variability captured = ",temp.varprop,"%)")
                    subtitle = base::paste0(self$variable,temp.string,temp.varprop,"%)")
                    );
              ...
              # temp.ylab <- base::gsub(x = temp.harmonic, pattern = "harmonic", replacement = "FPC ");
                temp.xlab <- ifelse(temp.index == base::length(temp.harmonics),"indice de date (1 = Jour de l'An)","");
                temp.ylab <- base::gsub(x = temp.harmonic, pattern = "harmonic", replacement = "CPF ");
